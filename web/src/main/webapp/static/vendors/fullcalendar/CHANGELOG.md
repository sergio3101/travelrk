...
- Fixes to Google Calendar API V3 code
	- wouldn't recognize a lone-string Google Calendar code if periods before the @ symbol
	- removeEventSource wouldn't work when given a Google Calendar code
...
- "dropping" external objects onto the calendar
	...
	- dropAccept (to categoryOfContent which events the calendar will accept)
	- drop (trigger)
