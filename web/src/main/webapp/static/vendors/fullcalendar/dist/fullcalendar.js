...
DragListener.mixin({
	...
	scrollIntervalId: null, // code of setTimeout for scrolling animation loop
...
function EventManager() { // assumed to be a calendar
	...
	// matchInput can either by a real event source object, an code, or the function/URL for the source.
	...
	function removeEvents(filter) {
		...
		else if (!$.isFunction(filter)) { // an event code
			eventID = filter + '';
	...
	function clientEvents(filter) {
		...
		else if (filter != null) { // not null, not undefined. an event code
			filter += '';
	...
	function mutateEvent(event, newProps, largeUnit) {
		...
		undoFunc = mutateEvents(
			clientEvents(event._id), // get events with this code
			clearEnd,
			newProps.allDay,
			startDelta,
			durationDelta,
	...
	//
	// TODO: don't use so many closures. possible memory issues when lots of events with same code.
	//
...
// 'businessHours'
// An event code (number or string)
...
Calendar.prototype.constraintToEvents = function(constraintInput) {
	...
	return this.clientEvents(constraintInput); // probably an code
};
